...
namespace Snap.CMC
{
    public partial class cmc_mat_stock_in : Form
    {
        static string constring = ConfigurationManager.ConnectionStrings["$safeprojectname$.Properties.Settings.erpConnectionString"].ConnectionString;
        MySqlConnection con = new MySqlConnection(constring);
        public cmc_mat_stock_in()
        {
            InitializeComponent();
        }

        private void cmc_mat_stock_in_Load(object sender, EventArgs e)
        {
        ...
        {
            MySqlDataAdapter da = new MySqlDataAdapter("select * from cmc_mat_stock_transaction where entry_type='IN'", con);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            MySqlDataAdapter da = new MySqlDataAdapter("select * from cmc_mat_stock_transaction where entry_type='IN' and date between '"+start_date.ToString()+"' and '"+end_date.ToString()+"'", con);
            DataTable dt = new DataTable();
            da.Fill(dt);
            foreach (DataRow dr in dt.Rows)
            {
