...
namespace Snap.CMC
{
    public partial class add_mat_stock : Form
    {
        static string constring = ConfigurationManager.ConnectionStrings["$safeprojectname$.Properties.Settings.erpConnectionString"].ConnectionString;
        MySqlConnection con = new MySqlConnection(constring);
        public add_mat_stock()
        {
            InitializeComponent();
        }

        private void add_mat_stock_Load(object sender, EventArgs e)
        {
        ...
        private void textBox5_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if(textBox5.Text=="")
                {
                    textBox6.Text = System.Convert.ToString(0 + System.Convert.ToDouble(textBox4.Text));
                }
                else
                {
                    textBox6.Text = System.Convert.ToString(System.Convert.ToDouble(textBox5.Text) + System.Convert.ToDouble(textBox4.Text));
                }
            }
            catch(Exception)
            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                cmd.CommandText = "insert into cmc_mat_stock_transaction (date,mat_code,uom,stock,entry_type) Values ('" + date.ToString() + "','" + textBox1.Text + "','" + textBox3.Text + "','" + textBox5.Text + "','IN')";
                cmd.ExecuteNonQuery();
