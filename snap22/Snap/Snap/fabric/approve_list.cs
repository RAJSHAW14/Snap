...
{
    public partial class approve_list : Form
    {
        ...
        private void approve_list_Load(object sender, EventArgs e)
        {
        ...
        {
            MySqlDataAdapter da = new MySqlDataAdapter("select * from fabric_than_details_view Where status='APPROVE'", con);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                cmd.CommandText = "update fabric_than_details set status='CUTTING' where id='" + id_value.ToString() + "'";
                cmd.ExecuteNonQuery();

                MessageBox.Show("Item Sent to Cutting", "Sucess", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                dataGridView1.Rows.Clear();
                MySqlDataAdapter da = new MySqlDataAdapter("select * from fabric_than_details_view Where status='APPROVE' and fabric_code like '%"+textBox1.Text+"%'", con);
                DataTable dt = new DataTable();
                da.Fill(dt);
                foreach (DataRow dr in dt.Rows)
                {
            ...
            {
                dataGridView1.Rows.Clear();
                MySqlDataAdapter da = new MySqlDataAdapter("select * from fabric_than_details_view Where status='APPROVE' and than_number like '%" + textBox1.Text + "%'", con);
                DataTable dt = new DataTable();
                da.Fill(dt);
                foreach (DataRow dr in dt.Rows)
                {
            ...
            {
                dataGridView1.Rows.Clear();
                MySqlDataAdapter da = new MySqlDataAdapter("select * from fabric_than_details_view Where status='APPROVE' and vendor like '%" + textBox1.Text + "%'", con);
                DataTable dt = new DataTable();
                da.Fill(dt);
                foreach (DataRow dr in dt.Rows)
                {
