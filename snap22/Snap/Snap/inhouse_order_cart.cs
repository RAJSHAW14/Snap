...
namespace Snap
{
    public partial class inhouse_order_cart : Form
    {
        static string constring = ConfigurationManager.ConnectionStrings["$safeprojectname$.Properties.Settings.erpConnectionString"].ConnectionString;
        MySqlConnection con = new MySqlConnection(constring);
        public inhouse_order_cart()
        {
            InitializeComponent();
        }

        private void inhouse_order_cart_Load(object sender, EventArgs e)
        {
        ...
        public void auto_complete_desinger()
        {
            MySqlCommand cmd = new MySqlCommand("select desinger_name from designer_master where desinger_name like '%" + textBox5.Text + "%'", con);
            MySqlDataReader dr = cmd.ExecuteReader();
            AutoCompleteStringCollection mycollection = new AutoCompleteStringCollection();
            while (dr.Read())
            {
        ...
        {
            MySqlDataAdapter da = new MySqlDataAdapter("select * from no_series where department='" + comboBox1.Text + "' and order_type='IN-HOUSE' AND table_for='emb_tracker'", con);
            DataTable dt = new DataTable();
            da.Fill(dt);
            foreach (DataRow dr in dt.Rows)
            {
            ...
            int check = 0;
            MySqlDataAdapter da2 = new MySqlDataAdapter("select * from emb_tracker where order_number='" + order_number + "' and order_type='IN-HOUSE'", con);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            emb_inhouse_List list = new emb_inhouse_List();
            this.Close();
