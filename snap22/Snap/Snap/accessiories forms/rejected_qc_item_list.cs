...
{
    public partial class rejected_qc_item_list : Form
    {
        static string constring = ConfigurationManager.ConnectionStrings["$safeprojectname$.Properties.Settings.erpConnectionString"].ConnectionString;
        MySqlConnection con = new MySqlConnection(constring);
        public rejected_qc_item_list()
        {
            InitializeComponent();
        }

        private void rejected_qc_item_list_Load(object sender, EventArgs e)
        {
        ...
        {
            MySqlDataAdapter da = new MySqlDataAdapter("select * from acc_qc_transaction_list where d_status='REJECTED' group by id_number", con);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                dataGridView1.Rows.Clear();
                MySqlDataAdapter da = new MySqlDataAdapter("select * from acc_qc_transaction_list where d_status='REJECTED' and product_code like '%"+textBox1.Text+"%' group by id_number", con);
                DataTable dt = new DataTable();
                da.Fill(dt);
                foreach (DataRow dr in dt.Rows)
                {
