...
namespace Snap
{
    public partial class non_fabirc_negative_stock : Form
    {
        static string constring = ConfigurationManager.ConnectionStrings["$safeprojectname$.Properties.Settings.erpConnectionString"].ConnectionString;
        MySqlConnection con = new MySqlConnection(constring);
        public non_fabirc_negative_stock()
        {
            InitializeComponent();
        }

        private void non_fabirc_negative_stock_Load(object sender, EventArgs e)
        {
        ...
        {
            MySqlDataAdapter da = new MySqlDataAdapter("select * from item where inventory<0", con);
